...
<script>
    function FillTheVoidPindahKamar(x){
        document.getElementById('idFillTheVoidPindahKamar').value = x
    }

    function RunTheButtonSearchPindahKamar(){
        document.getElementById('idButtonVoidPindahKamar').click();
    }

    function EnterRunningNowPindahKamar(event){
        if (event.key == 'Enter' || event.keyCode == 13){
            event.preventDefault();
            RunTheButtonSearchPindahKamar();
        }
    }
    </script>
<script>
    $(document).on("submit", "#frmViewDataPindahKamar", function(e) {
        e.preventDefault();
        var me = $(this),
        id = me.attr('id');
        var url = me.attr('action');
        var frmData = me.serialize();
        var valP = $('#' + id + ' input[name="namaPindahKamar"]').val();
        var not = {valP: valP}
        $.get("/pengelolaan-ajuan-pindah-kamar/dataTableViewPindahKamar", function(tbl) {
            $("#cardContentDataPindahKamar").html(tbl);
            $(tblViewDataPindahKamar(not));
        });
    });

    function tblViewDataPindahKamar(not) {
        var url = not == undefined ? "/pengelolaan-ajuan-pindah-kamar/isidataTableViewPindahKamar" : "/pengelolaan-ajuan-pindah-kamar/isidataTableViewPindahKamar?namaPindahKamar=" + encodeURIComponent(not.valP) ;
