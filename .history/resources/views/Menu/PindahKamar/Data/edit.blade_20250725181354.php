@foreach ($data as $x)
<form action="/pengelolaan-ajuan-pindah-kamar/update/{{ $x->id_penghuni }}" class="windowForm postClassX" id="editForm" dataTableId="#DataPindahKamarTableId" enctype="multipart/form-data" method="post">
    @csrf
    @method('put')
    <div class="card-body" style="padding: 3px;">
        <div class="row">
            ...
            <div class="mb-1 col-12">
                ...
                <select onkeydown="EnterButtonIsRunningNowPindahKamarEdit(event)" name="status" id="statusEdit" class="form-group form-control" aria-label="Default select example">
      ...
      <div class="row">
        <div class="col-12 mt-3" style="justify-content:flex-end; display:flex;">
            <button onclick="ButtonSumbitIsRunningPindahKamarEdit()" type="button" style="width:120px; text-align:center; font-weight:600; background-color: #2600FF;" class="btn btn-primary">Simpan</button>
            <button hidden type="submit" id="RealSubmitButtonPindahKamarEdit" style="width:120px; text-align:center; font-weight:600; background-color: #2600FF;" class="btn btn-primary">Simpan</button>
        </div>
      </div>
    </div>
</form>
@endforeach
<script>
    function EnterButtonIsRunningNowPindahKamarEdit(event){
        if(event.key == "Enter" || event.keyCode == 13){
            event.preventDefault();
            ButtonSumbitIsRunningPindahKamarEdit();
        }
    }
</script>
<script>
    ...
    function ButtonSumbitIsRunningPindahKamarEdit() {
        const status = document.getElementById('statusEdit').value.trim();

        if (status == '' || status == null) {
            showAlert("Harap isi Status Penghuni!");
        } else {
            document.getElementById('RealSubmitButtonPindahKamarEdit').click();
        }
    }
</script>