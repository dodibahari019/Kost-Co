...
<form action="/pengelolaan-ajuan-pindah-kamar/update/{{ $x->id_penghuni }}" class="windowForm postClassX" id="editForm" dataTableId="#DataPenghuniTableId" enctype="multipart/form-data" method="post">
    ...
    <div class="card-body" style="padding: 3px;">
        <div class="row">
            ...
            <div class="mb-1 col-6">
                <label for="nomorKara" class="form-label fw-semibold">Kamar Asal</label>
                <input readonly value="{{ $x->nomor_kamar }}" type="text" name="nomorKara" id="nomorKaraEdit" class="form-control">
            </div>

            <div class="mb-1 col-6">
                <label for="email" class="form-label fw-semibold">Kamar Asal</label>
                <input readonly value="{{ $x->email }}" type="text" name="email" id="emailEdit" class="form-control">
            </div>

            <div class="mb-1 col-12">
                <label for="alamat" class="form-label fw-semibold">Alamat</label>
                <textarea name="alamat" readonly id="alamatEdit" rows="2" class="form-control">{{ $x->alamat }}</textarea>
            </div>
