...
@endsection
@section('Title', 'Pengelolaan Sewa Kamar')
@section('tabView')
            ...
            <div class="card card-primary card-tabs">
                ...
                <div class="tab-content" id="custom-tabs-one-tabContent">
                    <div class="tab-pane fade show active" id="custom-tabs-one-pengajuanbpkb" role="tabpanel" aria-labelledby="custom-tabs-one-pengajuanbpkb-tab">
                        @include('Menu.SewaKamar.Data.index')
                    </div>
...
<script>
    var dataTableData = [
        {
            idTable : "#DataSewaKamarTableId",
            tblColumns : [0, 1, 2,3 ],
            dom: 'Bfrtip',
            btnNames : ['excelHtml5', 'pdfHtml5', 'print'],
            url : "/pengelolaan-sewa-kamar/getViewSewaKamar",
            method : "GET",
            noBtnPrint: true,
            bPaginate: true,
            responsive: true,
...
<script>
    $(document).ready(function(){
        var id = $('#cardContentDataSewaKamar'),
            url = "/pengelolaan-sewa-kamar/viewSewaKamar";
...
<script>
    $(document).on("submit", "#frmViewDataPenghuni", function(e) {
        ...
        var valP = $('#' + id + ' input[name="namaPenghuni"]').val();
        var not = {valP: valP}
        $.get("/pengelolaan-sewa-kamar/dataTableViewPenghuni", function(tbl) {
            $("#cardContentDataPenghuni").html(tbl);
            $(tblViewDataPenghuni(not));
        });
    });

    function tblViewDataPenghuni(not) {
        var url = not == undefined ? "/pengelolaan-sewa-kamar/isidataTableViewPenghuni" : "/pengelolaan-sewa-kamar/isidataTableViewPenghuni?namaPenghuni=" + encodeURIComponent(not.valP) ;
        var table = $("#DataSewaKamarTableId").DataTable({
            "responsive": false,
            "lengthChange": false,
            "autoWidth": true,
            "searching": false,
            ...
            "initComplete": function(settings, json) {
                table.buttons().container().appendTo('#DataSewaKamarTableId_wrapper .col-md-6:eq(0)');
            }
        });
    }
    </script>
