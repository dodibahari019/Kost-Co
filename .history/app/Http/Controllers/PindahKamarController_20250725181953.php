...
class PindahKamarController extends Controller
{
    ...
    public function isidataTableViewPindahKamar(Request $request)
    {
        ...
        if ($request->has('namaPindahKamar') && $request->namaPindahKamar != '') {
            $keyword = $request->namaPindahKamar;
            $query->where(function ($q) use ($keyword) {
                ...
                ->orWhere('pindahkamar.nomor_kamar_tujuan', 'LIKE', "%$keyword%")
                ->orWhere('pindahkamar.status_pindah', 'LIKE', "%$keyword%")
                ->orwhere('pindahkamar.catatan', 'LIKE', "%$keyword%");
            });
        }
     ...
     */
    public function edit(string $idPindah)
    {
        $idUser = Session::get('id_user');
        $data = PindahKamar::select([
            'pindahkamar.id_pindah',
        ...
        ->join('penghuni', 'penghuni.id_user', '=', 'kamar.id_penghuni')
        ->join('datausers', 'datausers.id_user', '=', 'penghuni.id_user')
        ->where('pindahkamar.id_pindah', $idPindah)
        ->get();
     ...
     */
    public function update(Request $request, string $idPindah)
    {
        try{
            $validated = $request->validate([
                'status' => 'required|in:Diajukan,Ditolak,Disetujui',
            ]);

            if($validated['status'] == 'Dis')

            PindahKamar::where('id_pindah', $idPindah)->update([
                'status_pindah' => $validated['status'],
            ]);
