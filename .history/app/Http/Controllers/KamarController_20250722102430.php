...
class KamarController extends Controller
{
    ...
    public function isidataTableViewKamar(Request $request)
    {
       $query = Kamar::select([
        ...
        ->addIndexColumn()
        ->addColumn('action', function ($row) {
            $main = '<button type="button" action="/pengelolaan-kamar/edit/'.$row->id_penghuni.'" style="width:30px; height:30px; display: flex; justify-content: center; align-items: center;" class="btn btn-warning bg-gradient-warning modal-crud" id="data-edit-form-btn" title="Data | Edit Form">
                <i class="nav-icon fas fa-edit"></i>
            </button>';
